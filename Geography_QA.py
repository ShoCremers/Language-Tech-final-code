...
def print_answer_count(query):
	...
	if len(answer) == 1 and answer[-1].isdigit(): # If there is only one answer and it is a number, most likey the answer (like population)
		print(answer[-1])
...
def find_question_number(line): #if there is a question number, it returns it and takes it out of the question.
	id_num = ''
	if line[0].isdigit():
		id_num = line[0]
		line.pop(0)
...
def get_property_W_prn(question, entity):
	...
	if verb_count == 1 and last_verb == "be": # questions like "what was ... of ..." or "who is ... of ..."
		for word in question:
			if verb_found == True:
...
def get_property_state(question, entity):
	...
	if prop: # make sure the last word is not plural (cities -> city)
		if last_word.tag_ == "NNS":
			prop[-1] = last_word.lemma_
		return prop
	else: # find a noun after entity
		det_found = False
		...
		if not prop:
			return prop
		else: # make sure the last word is not plural
			if last_word.tag_ == "NNS":
				prop[-1] = last_word.lemma_
			prop.append("of")
			return prop

# almost exactly the same as Wh det questions
# but we look for "many" instead of Wh-word	
...
# if entity is not a propernoun, look for the last noun
...
def get_entity(question): # find the propernoun from the sentence
	entity = []
	possible_entity = []
	NNP_found = False
	for word in question:
...
def QA(line): 
	...
	yes_words = ["can","be", "do"]
	h_words = ["how"]
	state_words = ["state", "name", "list", "report", "give"]
